...
<div class="content-area">
    <div class="card card-dark">
        <div class="card-header-dark">
            <h3 class="card-title-modern">Add Cyber Security Section</h3>
        </div>
        <div class="card-body">
            <form action="{{ route('cybers.store') }}" method="POST" enctype="multipart/form-data" id="cyberForm">
                ...
                <div class="form-group">
                    <label for="short_description" class="form-label-dark">Short Description</label>
                    <textarea name="short_description" id="short_description" class="form-control-dark @error('short_description') is-invalid @enderror" rows="2">{{ old('short_description') }}</textarea>
                    @error('short_description') <small class="text-danger">{{ $message }}</small> @enderror
                </div>

                <div class="form-group">
                    <label for="long_description" class="form-label-dark">Long Description</label>
                    <textarea name="long_description" id="long_description" class="form-control-dark @error('long_description') is-invalid @enderror" rows="5">{{ old('long_description') }}</textarea>
                    @error('long_description') <small class="text-danger">{{ $message }}</small> @enderror
                </div>

                <div class="form-group mb-3">
                    <label for="image" class="form-label-dark">Main Image</label>
                ...
                <div class="form-group mb-3">
                    <label for="frame_image" class="form-label-dark">Frame Image</label>
                    <div class="image-upload-container" id="frameImageUploadContainer">
                        ...
                        <input type="file" name="frame_image" id="frame_image" class="@error('frame_image') is-invalid @enderror" accept="image/*">
                ...
                <div class="d-flex justify-content-center gap-3">
                    <button type="submit" class="btn-modern-green"><i class="fas fa-save"></i> Save</button>
                    <a href="{{ route('cybers.index') }}" class="btn-modern-red"><i class="fas fa-times"></i> Back</a>
                </div>
            </form>
        </div>
    </div>
