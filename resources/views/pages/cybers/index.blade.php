        ...
        <div class="card-body p-0">
            <div class="table-responsive rounded rounded-lg">
                <table class="table align-middle table-dark table-hover mb-0">
                    ...
                    <tbody>
                        @forelse($cybers as $cyber)
                        <tr style="transition: background 0.2s ease;">
                            <td class="text-center" style="font-size: 0.875rem; color: var(--text-secondary);">{{ $cyber->id }}</td>
                            <td class="text-center">
                                @if($cyber->image)
                                    <div class="image-badge img-badge">
                                        <img src="{{ asset('storage/'.$cyber->image) }}" alt="Cyber Image" class="rounded-circle" width="48" height="48">
                                    </div>
                                @else
                                    <div class="image-badge user-badge">No Image</div>
                                @endif
                            </td>
                            <td class="fw-semibold text-center" style="font-size: 0.875rem; color: var(--text-primary);">{{ $cyber->title }}</td>
                            <td class="text-center" style="font-size: 0.875rem; color: var(--text-secondary);">{{ $cyber->subtitle }}</td>
                            <td class="text-center">
                                @if($cyber->frame_image)
                                    <div class="image-badge img-badge">
                                        <img src="{{ asset('storage/'.$cyber->frame_image) }}" alt="Frame Image" class="rounded-circle" width="48" height="48">
                                    </div>
                                @else
                                    <div class="image-badge user-badge">No Frame</div>
                                @endif
                            </td>
                            <td class="text-center">
                                @if(isset($cyber->is_active) && $cyber->is_active == 1)
                        ...
                        <tr>
                            <td colspan="7" class="text-center py-5" style="font-size: 0.875rem; color: var(--text-secondary);">
                                <i class="bi bi-info-circle me-1"></i> No sections found.
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
