...
<div class="content-area">
    <div class="card card-dark">
        <div class="card-header-dark">
            <h3 class="card-title-modern">Edit Story</h3>
        </div>
        <div class="card-body">
            <form action="{{ route('stories.update', $story->id) }}" method="POST" enctype="multipart/form-data" id="storyForm">
                @csrf
                @method('PUT')

                <div class="form-group">
                    <label for="title" class="form-label-dark">Title <span class="text-danger">*</span></label>
                    <input type="text" name="title" id="title" class="form-control-dark @error('title') is-invalid @enderror" required value="{{ old('title', $story->title) }}">
                    @error('title') <small class="text-danger">{{ $message }}</small> @enderror
                </div>

                <div class="form-group">
                    <label for="subtitle" class="form-label-dark">Subtitle</label>
                    <input type="text" name="subtitle" id="subtitle" class="form-control-dark @error('subtitle') is-invalid @enderror" value="{{ old('subtitle', $story->subtitle) }}">
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" id="description" class="form-control-dark @error('description') is-invalid @enderror" rows="4">{{ old('description', $story->description) }}</textarea>
                ...
                <!-- <div class="form-group mb-2">
                    ...
                    <div class="image-upload-container" id="imageUploadContainer">
                        <img id="imagePreview" class="image-preview" src="{{ $story->image_path ? asset('storage/'.$story->image_path) : '#' }}" style="{{ $story->image_path ? 'display:block;' : 'display:none;' }}">
                ...
                <div class="mb-4 text-center">
                    <label class="form-label-dark">Current Story Image</label>
                    @if ($event->hasMedia('stories'))
                        <img src="{{ $event->getFirstMediaUrl('stories') }}"
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="order_no" id="order_no" class="form-control-dark @error('order_no') is-invalid @enderror" value="{{ old('order_no', $story->order_no) }}">
                    @error('order_no') <small class="text-danger">{{ $message }}</small> @enderror
                </div>

                <div class="form-group">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ $story->is_active ? 'checked' : '' }}>
                        <label class="form-check-label" for="is_active">Active</label>
                    </div>
                </div>

                <div class="d-flex justify-content-center gap-3 pt-3">
                    <button type="submit" class="btn-modern-green"><i class="fas fa-save"></i> Update Story</button>
