...
<div class="content-area">
    <div class="card card-dark">
        <div class="card-header-dark">
            <h3 class="card-title-modern">Add Skill for: {{ $field->title }}</h3>
        </div>
        <div class="card-body">
            <form action="{{ route('technology.storeSkill', $field->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                    <label class="form-label-dark">Skill Name<span class="text-danger">*</span></label>
                    <input type="text" name="name" class="form-control-dark @error('name') is-invalid @enderror" value="{{ old('name') }}" required>
                    @error('name')<small class="text-danger">{{ $message }}</small>@enderror
                </div>

                <!-- Icon Upload -->
                <div class="form-group">
                    <label class="form-label-dark">Icon <span class="text-danger">*</span></label>
                    <div class="image-upload-container" id="iconUploadContainer">
                        <div class="upload-text">Drag & Drop Icon Here or Click to Upload</div>
                        <input type="file" name="icon" id="iconInput" accept="image/*" required>
                        <img id="iconPreview" class="image-preview">
                        @error('icon')<small class="text-danger">{{ $message }}</small>@enderror
                    </div>
                </div>

                <div class="form-group">
                    <label class="form-label-dark">Order</label>
                    <input type="number" name="order_no" class="form-control-dark @error('order_no') is-invalid @enderror" value="{{ old('order_no', 0) }}">
                    @error('order_no')<small class="text-danger">{{ $message }}</small>@enderror
                </div>

                <div class="d-flex justify-content-center gap-3">
                    <button type="submit" class="btn-modern-green"><i class="fas fa-save"></i> Add Skill</button>
                    <a href="{{ route('technology.editField', $field->id) }}" class="btn-modern-red"><i class="fas fa-times"></i> Back</a>
                </div>
            </form>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    const container = document.getElementById('iconUploadContainer');
