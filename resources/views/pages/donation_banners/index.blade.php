                    ...
                    <tbody>
                            ...
                            <td class="text-center">
                                {{-- @if($banner->image_path)
                                    <div class="image-badge img-badge">
                                        <img src="{{ asset('storage/'.$banner->image_path) }}" alt="Banner Image" class="rounded-circle" width="48" height="48">
                                    </div>
                                @else
                                ...
                                @if($banner->hasMedia('donationbanners'))
                                    <div class="image-badge img-badge">
                                        <img src="{{ $banner->getFirstMediaUrl('donationbanners') }}" alt="Donation Banner" class="rounded-circle" width="48" height="48">
                                    </div>
                                @else
                                    <div class="image-badge user-badge">No Image</div>
                                @endif
                            </td>
                            <td class="fw-semibold text-center" style="font-size: 0.875rem; color: var(--text-primary);">{{ $banner->section_title ?? '—' }}</td>
                            <td class="text-center" style="font-size: 0.875rem; color: var(--text-secondary);">{{ Str::limit($banner->main_quote, 50) }}</td>
                            <td class="text-center" style="font-size: 0.875rem; color: var(--text-secondary);">{{ $banner->button_text ?? '—' }}</td>
                            <td class="text-center">
                        ...
                        <tr>
                            <td colspan="7" class="text-center py-5" style="font-size: 0.875rem; color: var(--text-secondary);">
                                <i class="bi bi-info-circle me-1"></i> No banners found.
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
...
<style>
    ...
    .title-underline { background: linear-gradient(to right, var(--primary-color), var(--secondary-color)) !important; height: 4px !important; border-radius: 4px !important; }
    ...
    .add-btn:hover { background-color: var(--secondary-color) !important; transform: translateY(-3px); box-shadow: var(--shadow-hover); }
    ...
    @media (max-width: 767.98px) {
        .title-header { font-size: 1.5rem !important; }
        .add-btn { font-size: 0.8rem !important; padding: 0.5rem 1rem !important; }
