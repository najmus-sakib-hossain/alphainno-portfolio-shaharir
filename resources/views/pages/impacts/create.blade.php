...
<div class="content-area">
    <div class="card card-dark">
        <div class="card-header-dark">
            <h3 class="card-title-modern">Add New Impact</h3>
        </div>
        <div class="card-body">
            <form action="{{ route('impacts.store') }}" method="POST" enctype="multipart/form-data" id="impactForm">
                ...
                <div class="form-group">
                    ...
                    <select name="type" id="type" class="form-control-dark @error('type') is-invalid @enderror" required>
                ...
                <div class="form-group">
                    ...
                    <div class="image-upload-container mb-3" id="imageUploadContainer{{ $i }}">
                        <div class="upload-text">Drag & Drop Image {{ $i }} Here or Click to Upload</div>
                        <input type="file" name="image{{ $i }}" id="image{{ $i }}" accept="image/*">
                        <img id="preview{{ $i }}" class="image-preview" alt="Image {{ $i }} Preview">
                ...
                <div class="form-group">
                    ...
                    <div id="points-wrapper">
                        <input type="text" name="points[]" class="form-control-dark mb-2" placeholder="Enter impact point" value="{{ old('points.0') }}">
                    </div>
                    <button type="button" class="btn-modern-green" onclick="addPoint()">+ Add Point</button>
                </div>
                ...
                <div class="d-flex justify-content-center gap-3">
                    <button type="submit" class="btn-modern-green"><i class="fas fa-save"></i> Save Impact</button>
                    <a href="{{ route('impacts.index') }}" class="btn-modern-red"><i class="fas fa-times"></i> Cancel</a>
                </div>
            </form>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    function setupImageUpload(containerId, inputId, previewId) {
        ...
        container.addEventListener('click', e => {
            // Only trigger if the actual container (not a child input) was clicked
        ...
        container.addEventListener('drop', e => {
            ...
            handleImage(e.dataTransfer.files[0], preview, container, uploadText);
        });

        input.addEventListener('change', () => {
            if (input.files.length > 0) {
                handleImage(input.files[0], preview, container, uploadText);
            }
        });
    }

    function handleImage(file, preview, container, uploadText) {
    ...
    // Setup all 4 images
    for (let i = 1; i <= 4; i++) {
        setupImageUpload('imageUploadContainer'+i, 'image'+i, 'preview'+i);
    }

    // Add impact points dynamically
